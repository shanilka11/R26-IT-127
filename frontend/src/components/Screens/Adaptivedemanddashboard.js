...
const NODE_API_BASE = `${LocalIP}:5000`;
...
const AdaptiveDemandDashboard = () => {
  ...
  const fetchActualDemand = async (service, forDate) => {
    try {
      const res = await axios.get(`${NODE_API_BASE}/actual_demand`, {
        params: {
      ...
      if (res.data && res.data.success) {
        setActualByClass(res.data.actual_demand_by_class);
  ...
  return (
    <div className="add-page">
      <div className="add-header">
        <h1 className="add-heading">Adaptive Demand Dashboard</h1>
        <p className="add-subheading">Predicted vs actual demand, and seat allocation, per travel class</p>
      </div>
            ...
            <div className="add-donut-wrap">
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  ...
                  <Tooltip
                    contentStyle={{ background: '#131a2b', border: '1px solid #2a3450', borderRadius: 8 }}
          ...
          <div className="add-card add-chart-card">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={actualVsPredictedData} barGap={6}>
                <CartesianGrid strokeDasharray="3 3" stroke="#232b40" vertical={false} />
                <XAxis dataKey="class" stroke="#8891a8" />
                <YAxis stroke="#8891a8" />
                <Tooltip
                  contentStyle={{ background: '#131a2b', border: '1px solid #2a3450', borderRadius: 8 }}
                />
